...
local function fetch_ts_references(bufnr)
...
local function fetch_lsp_references(bufnr, lsp_params)
  ...
  if #clients == 0 then
    set_error("[rename] No active language server with rename capability")
    return
  end
  ...
  vim.lsp.buf_request(bufnr, "textDocument/references", params, function(err, result, _, _)
    if err then
      set_error("[rename] Error while finding references: " .. err.message, vim.lsp.log_levels.ERROR)
      return
    end
    if not result or vim.tbl_isempty(result) then
      set_error("[rename] Nothing to rename", vim.lsp.log_levels.WARN)
      return
    end
